...
为了提升稳定性，设备端在连接失败或发布失败时会断开并重新进入 `begin` 标签流程，确保 Wi-Fi 与 MQTT 能够重新建立；消息发送时设置 QoS=1，保证至少一次送达（允许重复但不丢失），适合环境数据上报场景。

**设备端 Wi-Fi 与 MQTT 连接代码：**
...
在上报逻辑中，温湿度值以 1 位小数格式化，避免数据过长；此外使用 `memset` 清空 payload 缓冲，避免旧数据残留导致解析错误。
...
此外，回调中对 payload 进行了两次换行符剔除，兼容不同客户端可能发送的 `\n` 或 `\r\n` 结尾，避免命令解析失败。

### 2.4 实验现象记录（设备端）

![设备端终端输出截图](image-1.png)
...
为避免阻塞 MQTT 回调线程，数据库写入采用新线程异步执行；如果解析失败，会在日志中输出错误信息，便于定位格式问题或设备端异常。
...
前端触发控制后，服务端会立即更新缓存状态并返回成功响应，使 UI 状态与设备端执行保持同步；若 MQTT 未连接，接口返回 500 并提示错误原因，便于前端提示用户重试。
...
数据库连接使用 `utf8mb4` 编码，避免日志或设备 ID 中包含非 ASCII 字符时写入失败。
...
接口层仅返回绘图所需字段，减少前端处理量；同时按时间升序排序，避免 ECharts 图表出现折线回跳。
...
为提高交互体验，前端在禁用按钮期间仍显示上一次状态，待服务器响应成功后再更新灯光状态，避免按钮频繁闪烁。
...
实时模式下会周期性调用 `/api/devices`，仅更新当前选中设备的数据，以降低页面渲染成本并保证图表流畅。
### 5.4 实验现象记录（Web 前端）

- 设备列表与实时数据页面截图：

  ![设备列表与实时数据页面截图](1768654294152.png)

- 灯光控制按钮执行结果截图：

  ![灯光控制按钮执行结果截图](6a251bef3b9174c787b86c04703051d5.jpg)

- 历史数据回放曲线截图：

  ![历史数据回放曲线截图](1768654333339.png)

- 服务端网页首页截图（运行 `python server.py` 后访问 `http://localhost:5000`）：
